"""Neutral deterministic V2 oracle schema shared by capture and verification."""
...
DETERMINISTIC_ARTIFACT_KIND_V2 = "deterministic_observations_v2"
DETERMINISTIC_ORACLE_DOMAIN_V2 = b"cv-trust-agent/deterministic-oracle/v2\0"
_MAX_ORACLE_BYTES = 2 * 1024 * 1024


class OracleSpecV2Error(ValueError):
    """The neutral V2 oracle schema is invalid."""
...
class RankKeyExpectationV2(_StrictModel):
...
class RouteExpectationV2(_StrictModel):
    ...
    rank_key: RankKeyExpectationV2 | None = None

    @model_validator(mode="after")
    def rank_fields_are_atomic(self) -> RouteExpectationV2:
        allowed_pairs = {
...
class ExplanationExpectationV2(_StrictModel):
    template: Literal[
        "record_degraded",
        "record_quarantined",
...
class ExactExpectationV2(_StrictModel):
    kind: Literal["exact"]
    decision_semantic_sha256: Annotated[
        str,
        StringConstraints(pattern=r"^[0-9a-f]{64}$"),
    ]
    strategy: Token
    ranking_scope: Literal["COMPLETE", "PARTIAL", "NONE"]
    routes: tuple[RouteExpectationV2, ...] = Field(min_length=10, max_length=10)
    explanations: tuple[ExplanationExpectationV2, ...] = Field(max_length=50)
    ...
    @model_validator(mode="after")
    def exact_routes_are_complete(self) -> ExactExpectationV2:
        strategy_scope = {
...
class EqualExpectationV2(_StrictModel):
    kind: Literal["equal_to"]
    reference: Token


CaseExpectationV2: TypeAlias = ExactExpectationV2 | EqualExpectationV2


class CaseOracleV2(_StrictModel):
    name: Token
    fixture_id: Token
    fixture_tree_sha256: Annotated[str, StringConstraints(pattern=r"^[0-9a-f]{64}$")]
    showcase: bool
    expectation: CaseExpectationV2 = Field(discriminator="kind")


class InvariantOracleV2(_StrictModel):
    name: Token
    kind: Literal[
        "projection_equal",
        "route_equal_except",
    ...
    @model_validator(mode="after")
    def operands_match_kind(self) -> InvariantOracleV2:
...
class DeterministicOracleV2(_StrictModel):
    schema_version: Literal[2]
    suite_id: Token
    cases: tuple[CaseOracleV2, ...] = Field(min_length=1, max_length=64)
    invariants: tuple[InvariantOracleV2, ...] = Field(min_length=1, max_length=128)

    @model_validator(mode="after")
    def references_are_closed(self) -> DeterministicOracleV2:
        ...
        for case in self.cases:
            if isinstance(case.expectation, EqualExpectationV2):
...
def oracle_sha256_v2(oracle: DeterministicOracleV2) -> str:
    return hashlib.sha256(
        DETERMINISTIC_ORACLE_DOMAIN_V2
        + canonical_json_bytes(oracle.model_dump(mode="json", exclude_none=False))
    ).hexdigest()


def load_deterministic_oracle_v2(path: Path) -> DeterministicOracleV2:
    raw = load_strict_json_object(path, maximum_bytes=_MAX_ORACLE_BYTES)
    try:
        return DeterministicOracleV2.model_validate_json(canonical_json_bytes(raw))
    except Exception as exc:
        raise OracleSpecV2Error("deterministic V2 oracle is invalid") from exc
