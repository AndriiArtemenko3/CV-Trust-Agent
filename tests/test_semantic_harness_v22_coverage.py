"""Hostile coverage for the independently derived V2.2 release semantics.
...
from evaluation.aggregate_v22 import AggregateV22Error, validate_aggregate_v22
...
from evaluation.release_spec_v2 import (
    implementation_tree_sha256_v2 as implementation_tree_sha256_v2_actual,
)
from evaluation.release_spec_v22 import (
    CommandV22,
    CorroborationRequestV22,
    ...
    implementation_tree_sha256_v22,
...
from evaluation.slot_ledger_v22 import ValidatedSlotLedgerV22
...
@pytest.fixture(scope="module")
def supported_projection() -> DecisionProjectionV22:
...
@pytest.fixture(scope="module")
def partial_projection() -> DecisionProjectionV22:
...
@pytest.fixture(scope="module")
def hold_projection() -> DecisionProjectionV22:
...
def _deterministic_release_fixture(
    ...
    return ValidatedDeterministicReleaseV22(
        ...
        fixture_commitments=(
            ...
            ("structured_note_poisoned", "b" * 64),
        ),
        fixture_ids=(
            ("clean", "canonical_clean"),
            ("structured_note_directive", "canonical_structured_note_directive"),
            ("structured_note_poisoned", "canonical_structured_note_poisoned"),
        ),
    )
            ...
            for order_index, condition in enumerate(order, start=1):
                ...
                for candidate in oracle.candidates:
                    ...
                    for claim in claims:
                        ...
                        kind_counts[kind if kind in kind_counts else "unknown_kind"] += 1
                    candidates.append(
                        {
...
def _naive_attempt_rows() -> list[Json]:
    ...
    for block_id, (seed, schedule) in enumerate(
        zip(NAIVE_SEEDS_V22, LATIN_SQUARE_SCHEDULE_V22, strict=True),
        start=1,
    ):
...
def _secure_release_fixture() -> ValidatedSecureReleaseV22:
    configuration = (
        SecureArmConfigurationV22(
            arm="canonical",
            model_identifier="model_canonical",
            sdk_version="sdk_1.0",
            prompt_sha256=CANONICAL_SECURE_PROMPT_SHA256_V22,
        ...
        SecureArmConfigurationV22(
            ...
            sdk_version="sdk_1.0",
            prompt_sha256=HELDOUT_SECURE_PROMPT_SHA256_V22,
    ...
    exact_counts = tuple((f"AP-10{index}", 3) for index in range(1, 5))
    recall = tuple((f"AP-10{index}", 4, 4) for index in range(1, 5))
    return ValidatedSecureReleaseV22(
        ...
        canonical_provider_success_count=60,
        canonical_bound_count=6,
        canonical_audit_valid_count=6,
        canonical_evaluable_pair_count=3,
        canonical_noninterference_pair_count=3,
        prose_valid_nonempty_candidate_count=24,
        unsupported_claim_count=0,
        promotion_count=0,
        clean_utility_run_count=3,
        candidate_exact_clean_counts=exact_counts,
        candidate_fact_recall=recall,
        candidate_span_recall=recall,
...
def _naive_release_fixture() -> ValidatedNaiveReleaseV22:
    metrics = NaiveSeriesMetricsV22(
        pair_count=8,
...
def _patch_aggregate_dependencies(
    ...
    *,
    secure: ValidatedSecureReleaseV22 | None = None,
    naive: ValidatedNaiveReleaseV22 | None = None,
    current_hash: str = "c" * 64,
    secure_ledger_counts: tuple[int, int, int] = (84, 0, 0),
) -> None:
    monkeypatch.setattr(
        aggregate_v22,
        "validate_deterministic_release_v22",
    ...
    monkeypatch.setattr(
        ...
        lambda _path, *, artifact_path, ledger_kind: ValidatedSlotLedgerV22(
            run_id="v24-20260817-r1",
            ledger_kind=ledger_kind,
            artifact_sha256=("1" if ledger_kind == "secure" else "2") * 64,
            final_chain_sha256=("3" if ledger_kind == "secure" else "4") * 64,
            slot_count=84 if ledger_kind == "secure" else 32,
            completed_count=(secure_ledger_counts[0] if ledger_kind == "secure" else 32),
            failed_count=(secure_ledger_counts[1] if ledger_kind == "secure" else 0),
            unobserved_count=(secure_ledger_counts[2] if ledger_kind == "secure" else 0),
        ),
    )
    monkeypatch.setattr(aggregate_v22, "release_implementation_paths_v2", lambda _root: ())
    monkeypatch.setattr(
        aggregate_v22,
...
def _model_payload(projection: DecisionProjectionV22, name: str) -> Json:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("graph", SupportGraphV22, lambda item: item["evidence_ids"].pop(), "manifest must equal"),
        (
            "graph",
            SupportGraphV22,
        ...
        ("graph", SupportGraphV22, _graph_unknown_evidence, "evidence outside"),
        ("graph", SupportGraphV22, _graph_unknown_fact, "unknown fact"),
        ("graph", SupportGraphV22, _graph_unknown_feature, "unknown feature"),
        (
            "graph",
            SupportGraphV22,
        ...
        ("graph", SupportGraphV22, _graph_cycle, "must be acyclic"),
        (
            "route",
            RouteV22,
        ...
        ("route", RouteV22, lambda item: item["evidence_ids"].pop(), "evidence differs"),
    ],
)
def test_v22_component_models_fail_closed_on_invalid_local_semantics(
    projection: DecisionProjectionV22,
    model_name: str,
    model: type[Any],
...
def test_evidence_model_independently_rejects_nonfinite_page_dimensions(
    ...
    with pytest.raises(Exception, match="page dimensions must be finite"):
        EvidenceRefV22.model_validate(payload)


def test_v22_component_models_reject_nonfinite_values_and_bounded_requests(
    projection: DecisionProjectionV22,
) -> None:
    ...
    with pytest.raises(Exception, match="bounds must be finite"):
        EvidenceRefV22.model_validate(evidence)
    ...
    with pytest.raises(Exception, match="fact values must be finite"):
        SupportedFactV22.model_validate(fact)
    ...
    with pytest.raises(Exception, match="feature values must be finite"):
        DerivedFeatureV22.model_validate(feature)

    with pytest.raises(Exception, match="unknown bounded value"):
        CorroborationRequestV22.model_validate_json(
            canonical_json_bytes(
                {
    ...
    with pytest.raises(Exception, match="unknown reason code"):
        ExplanationV22.model_validate_json(
            canonical_json_bytes(
                {
...
def _change_route_snapshot_consistently(value: Json) -> None:
    ...
    for reference in cast(list[Json], graph["evidence_manifest"]):
        if reference["source_kind"] != "application_json":
            continue
        original = cast(str, reference["evidence_id"])
        role = cast(str, reference["field_path"]).rsplit(".", maxsplit=1)[-1]
        replacement = _canonical_evidence_id_v22(
            "json",
            "stale_snapshot",
            cast(str, reference["candidate_id"]),
            cast(str, reference["semantic_hash"]),
            role,
        )
        reference["evidence_id"] = replacement
        rewritten_ids[original] = replacement
    graph["evidence_ids"] = sorted(
        rewritten_ids.get(item, item) for item in cast(list[str], graph["evidence_ids"])
    )
    route["evidence_ids"] = list(cast(list[str], graph["evidence_ids"]))
    for fact in cast(list[Json], graph["facts"]):
        fact["evidence_ids"] = sorted(
            rewritten_ids.get(item, item) for item in cast(list[str], fact["evidence_ids"])
        )
...
def test_complete_projection_validator_rejects_execution_and_release_tampering(
    ...
    mutation(raw)
    with pytest.raises(ReleaseSpecV22Error) as caught:
        DecisionProjectionV22.from_canonical(raw)
...
def test_replanned_projection_rejects_diff_and_corroboration_tampering(
    supported_projection: DecisionProjectionV22,
    partial_projection: DecisionProjectionV22,
    hold_projection: DecisionProjectionV22,
    fixture_name: str,
    mutation: Mutation,
    message: str,
) -> None:
...
def _make_cross_candidate_parent(value: Json) -> None:
    ...
    inventory = cast(Json, target["evidence_inventory"])
    inventory["candidate_id"] = "AP-001"
    for anchor in (
        "record_ap_years_reference",
        "record_invoice_processing_reference",
    ):
        reference = cast(Json, inventory[anchor])
        reference["candidate_id"] = "AP-001"
        reference["evidence_id"] = cast(str, reference["evidence_id"]).replace(
            ":AP-002:", ":AP-001:"
        )
        reference["field_path"] = cast(str, reference["field_path"]).replace("[AP-002]", "[AP-001]")
    for structured_anchor in cast(list[Json], inventory["structured_anchors"]):
        reference = cast(Json, structured_anchor["reference"])
        reference["candidate_id"] = "AP-001"
        reference["evidence_id"] = cast(str, reference["evidence_id"]).replace(
            ":AP-002:", ":AP-001:"
        )
    ...
    for entry in cast(list[Json], inventory["entries"]):
        cast(Json, entry["reference"])["candidate_id"] = "AP-001"
...
def test_projection_round_trip_and_digest_are_release_derived(
    ...
    assert DecisionProjectionV22.from_canonical(stored).digest() == projection.digest()
    assert (
        projection.digest()
        == hashlib.sha256(
            b"cv-trust-agent/decision-projection/v3\0" + canonical_json_bytes(stored)
        ).hexdigest()
    )
    ...
    assert DecisionProjectionV22.from_observation(producer_claims).digest() == projection.digest()
...
def test_all_four_runtime_strategies_have_independently_valid_release_projections(
    ...
    for item in projections:
        assert (
            DecisionProjectionV22.from_canonical(item.canonical_object()).digest() == item.digest()
        )
...
def test_fixed_exact_commitment_rejects_coordinated_self_consistent_graph_relabel(
    ...
    with pytest.raises(
        DeterministicReleaseV22Error,
        match=r"invalid decision projection|action semantics differ",
    ):
...
def test_deterministic_semantics_rejects_oracle_and_artifact_tampering(
    ...
    if root_name == "oracle":
        validated_oracle = DeterministicOracleV22.model_validate_json(canonical_json_bytes(oracle))
        artifact["oracle_sha256"] = oracle_sha256_v22(validated_oracle)
        oracle_path.write_bytes(canonical_json_bytes(oracle) + b"\n")
    artifact_path.write_bytes(canonical_json_bytes(artifact) + b"\n")

    with pytest.raises(DeterministicReleaseV22Error, match=message):
        validate_deterministic_semantics_v22(artifact_path, oracle_path)
...
def test_deterministic_structure_rejects_duplicate_cases_and_bad_projection(
    ...
    artifact_path.write_bytes(canonical_json_bytes(artifact) + b"\n")
    with pytest.raises(DeterministicReleaseV22Error, match="repeat a case"):
    ...
    artifact_path.write_bytes(canonical_json_bytes(artifact) + b"\n")
    with pytest.raises(DeterministicReleaseV22Error, match="invalid decision projection"):
        validate_deterministic_structure_v22(artifact_path)
...
def test_secure_semantics_derive_safety_utility_and_pair_evaluability(
    ...
    projection = ten_candidate_projection
...
def test_secure_and_naive_jsonl_reject_duplicate_keys(
    ...
    secure_artifact = tmp_path / "secure-v22.jsonl"
    secure_artifact.write_bytes(b"\n".join(secure_lines) + b"\n")
    with pytest.raises(SecureReleaseV22Error, match="not strict JSON"):
        validate_secure_structure_v22(secure_artifact)
...
def test_secure_semantics_counts_unsupported_claims_and_mapper_failures(
    ...
    candidates[1] = {
        ...
        "claim_kind_counts": dict.fromkeys(
            (
                "candidate_id",
                "ap_years",
                "invoice_processing",
...
def test_secure_semantics_rejects_protocol_and_result_tampering(
    ...
    mutation(rows)
    artifact = _write_jsonl(tmp_path / "secure-v22.jsonl", rows)
    with pytest.raises(SecureReleaseV22Error, match=message):
        validate_secure_semantics_v22(
            artifact,
            deterministic_release=_deterministic_release_fixture(projection),
            heldout_oracle_path=heldout_oracle_path,
...
def test_naive_semantics_rederive_counterbalanced_rank_effect_metrics(
    ...
    artifact = _write_jsonl(tmp_path / "naive-v22.jsonl", _naive_attempt_rows())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda rows: [
                row.__setitem__("attack_cohort_sha256", NAIVE_CLEAN_COHORT_SHA256_V22)
                or row.__setitem__(
                    "input_cohort_sha256",
                    NAIVE_CLEAN_COHORT_SHA256_V22,
                )
...
def test_naive_semantics_rejects_protocol_tampering(
    ...
    mutation(rows)
    artifact = _write_jsonl(tmp_path / "naive-v22.jsonl", rows)
    with pytest.raises(NaiveReleaseV22Error, match=message):
...
def test_naive_structure_rejects_self_inconsistent_rows(
    ...
    artifact = _write_jsonl(tmp_path / "naive-v22.jsonl", rows)
    with pytest.raises(NaiveReleaseV22Error, match="row is invalid"):
        validate_naive_structure_v22(artifact)


def _write_placeholder_v22_artifacts(evidence_directory: Path) -> None:
    evidence_directory.mkdir(parents=True)
    (evidence_directory / "deterministic-v22.json").write_bytes(b"deterministic\n")
    (evidence_directory / "secure-v22.jsonl").write_bytes(b"secure\n")
    (evidence_directory / "naive-v22.jsonl").write_bytes(b"naive\n")
    (evidence_directory / "secure-slots-v22.jsonl").write_bytes(b"secure-slots\n")
    (evidence_directory / "naive-slots-v22.jsonl").write_bytes(b"naive-slots\n")
...
def test_public_aggregate_rejects_substitute_oracle_paths(
    ...
    deterministic = tmp_path / "evaluation" / "oracle_v22.json"
    heldout = tmp_path / "evaluation" / "heldout_release_oracle_v22.json"
    if substituted == "deterministic":
        deterministic = tmp_path / "substitute-deterministic.json"
    else:
        heldout = tmp_path / "substitute-heldout.json"

    with pytest.raises(AggregateV22Error, match="canonical repository oracles"):
        if operation == "write":
            aggregate_v22.write_release_manifest_v22(
                tmp_path / "v24-20260817-r1",
                run_id="v24-20260817-r1",
            ...
            validate_aggregate_v22(
                tmp_path / "evidence" / "manifest-v22.json",
                deterministic_oracle_path=deterministic,
                heldout_oracle_path=heldout,
                repository_root=tmp_path,
            )
...
def test_aggregate_writer_and_validator_bind_all_transitive_components(
    ...
    _write_placeholder_v22_artifacts(evidence_directory)
    ...
    manifest = aggregate_v22.write_release_manifest_v22(
        evidence_directory,
        run_id="v24-20260817-r1",
        deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
        heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
        repository_root=tmp_path,
...
def test_aggregate_rejects_implementation_drift_during_property_execution(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    projection: DecisionProjectionV22,
) -> None:
    ...
    expected_hash = implementation_tree_sha256_v2_actual(
        (implementation,),
        repository_root=tmp_path,
    )
    ...
    monkeypatch.setattr(
        aggregate_v22,
        "implementation_tree_sha256_v2",
        implementation_tree_sha256_v2_actual,
    )
    evidence_directory = tmp_path / "v24-20260817-r1"
    _write_placeholder_v22_artifacts(evidence_directory)
    manifest = aggregate_v22.write_release_manifest_v22(
    ...
    monkeypatch.setattr(aggregate_v22, "_execute_property_gate_families", mutate_tree)
    with pytest.raises(AggregateV22Error, match="changed during property-gate execution"):
        validate_aggregate_v22(
            manifest,
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
            heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
...
def test_integrity_manifest_can_record_but_release_rejects_failed_secure_hard_gate(
    ...
    integrity_only = validate_aggregate_v22(
        manifest,
        deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
        heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
        repository_root=tmp_path,
    ...
    assert integrity_only.integrity_valid
    assert not integrity_only.release_green


def test_integrity_manifest_preserves_failed_secure_slots_but_cannot_be_green(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    projection: DecisionProjectionV22,
) -> None:
    ...
    assert integrity_only.integrity_valid
    assert not integrity_only.release_green
    assert integrity_only.secure.hard_gate_passed
    assert integrity_only.secure_ledger.failed_count == 1
...
def test_aggregate_writer_rejects_missing_stale_and_unbound_inputs(
    ...
    deterministic = _deterministic_release_fixture(projection)
    _patch_aggregate_dependencies(monkeypatch, deterministic)
    with pytest.raises(AggregateV22Error, match="exact five artifacts"):
        aggregate_v22.write_release_manifest_v22(
            tmp_path / "v24-20260817-r1",
            run_id="v24-20260817-r1",
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
    ...
    with pytest.raises(AggregateV22Error, match="stale or use mixed"):
        aggregate_v22.write_release_manifest_v22(
            evidence_directory,
            run_id="v24-20260817-r1",
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
    ...
    with pytest.raises(AggregateV22Error, match="naïve fixtures"):
        aggregate_v22.write_release_manifest_v22(
            evidence_directory,
            run_id="v24-20260817-r1",
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
    ...
    _patch_aggregate_dependencies(monkeypatch, deterministic)
    with pytest.raises(AggregateV22Error, match="timezone aware"):
        aggregate_v22.write_release_manifest_v22(
            evidence_directory,
            run_id="v24-20260817-r1",
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
...
def test_aggregate_validator_rejects_hash_or_commitment_tampering(
    ...
    with pytest.raises(AggregateV22Error, match="hash differs"):
        validate_aggregate_v22(
            manifest,
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
            heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
...
def test_aggregate_validator_rejects_manifest_binding_changes(
    ...
    raw[field] = "9" * 64
    manifest.write_bytes(canonical_json_bytes(raw) + b"\n")
    with pytest.raises(AggregateV22Error, match=message):
        validate_aggregate_v22(
            manifest,
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
            heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
...
def test_aggregate_validator_rejects_configuration_gate_and_freshness_failures(
    ...
    manifest.write_bytes(canonical_json_bytes(raw) + b"\n")
    with pytest.raises(AggregateV22Error, match="configuration differs"):
        validate_aggregate_v22(
            manifest,
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
            heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
    ...
    with pytest.raises(AggregateV22Error, match="stale relative"):
        validate_aggregate_v22(
            manifest,
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
            heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
    ...
    _patch_aggregate_dependencies(monkeypatch, deterministic)
    monkeypatch.setattr(
        aggregate_v22, "_execute_property_gate_families", lambda _root: ("only_one",)
    )
    with pytest.raises(AggregateV22Error, match="not completely executed"):
        validate_aggregate_v22(
            manifest,
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
            heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
    ...
    with pytest.raises(AggregateV22Error, match="invariant count is incomplete"):
        validate_aggregate_v22(
            manifest,
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
            heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
...
def test_property_gate_runner_accounts_for_success_and_failure(
    ...
    monkeypatch.setattr(aggregate_v22, "execute_property_gate_nodes", lambda *_args: ())
    assert aggregate_v22._execute_property_gate_families(tmp_path) == (
        "unseen_identity_renaming_and_input_permutation",
        "unseen_value_equivalence_and_composed_transform",
    )
    ...
    monkeypatch.setattr(aggregate_v22, "execute_property_gate_nodes", fail)
    with pytest.raises(AggregateV22Error, match="family failed"):
        aggregate_v22._execute_property_gate_families(tmp_path)
...
@pytest.mark.parametrize(
    ("validator", "error"),
    [
        (validate_deterministic_structure_v22, DeterministicReleaseV22Error),
        (validate_secure_structure_v22, SecureReleaseV22Error),
        (validate_naive_structure_v22, NaiveReleaseV22Error),
    ],
)
def test_v22_artifact_readers_reject_missing_empty_and_malformed_inputs(
    tmp_path: Path,
...
def test_structural_readers_reject_unknown_or_incomplete_records(tmp_path: Path) -> None:
    ...
    with pytest.raises(DeterministicReleaseV22Error):
        validate_deterministic_structure_v22(deterministic)
    ...
    with pytest.raises(SecureReleaseV22Error, match="event is not allowed"):
        validate_secure_structure_v22(secure)

    naive = tmp_path / "naive.jsonl"
    naive.write_text("{}\n", encoding="utf-8")
    with pytest.raises(NaiveReleaseV22Error, match="row is invalid"):
        validate_naive_structure_v22(naive)


def test_aggregate_rejects_manifest_without_exact_release_schema(tmp_path: Path) -> None:
    manifest = tmp_path / "manifest-v22.json"
    manifest.write_text("{}", encoding="utf-8")
    with pytest.raises(AggregateV22Error, match="manifest is invalid"):
        validate_aggregate_v22(
            manifest,
            deterministic_oracle_path=tmp_path / "evaluation" / "oracle_v22.json",
            heldout_oracle_path=tmp_path / "evaluation" / "heldout_release_oracle_v22.json",
...
def test_implementation_tree_hash_is_stable_and_path_bound(tmp_path: Path) -> None:
    ...
    first = implementation_tree_sha256_v22((source,), repository_root=tmp_path)
    second = implementation_tree_sha256_v22((source,), repository_root=tmp_path)
    assert first == second
    (source / "a.py").write_text("x = 2\n", encoding="utf-8")
    assert implementation_tree_sha256_v22((source,), repository_root=tmp_path) != first
